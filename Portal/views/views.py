...
def LoginPageView(request):
    ...
    if request.method == "POST":
        if request.POST.get('user_type') == "job_seeker":
            ...
            print("\n USER : ", user)
            if not user:
                return render(request, "login.html", {"msg"+"_"+request.POST.get('user_type'):"Email id does not exist"})
            
            if user[0].password != request.POST.get('password'):
                return render(request, "login.html", {"msg"+"_"+request.POST.get('user_type'):"Password is incorrect"})
    ...
    try:
        if request.method == "POST":
            ...
            if request.POST.get('password') != request.POST.get('cpwd'):                
                return render(request, "register.html", {"msg"+"_"+request.POST.get('user_type'):"Password and Confirm Password Must be same"})
            ...
            if request.POST.get('user_type') == "job_seeker":
                ...
                user = JobSeekerTbl.objects.filter(email=request.POST.get('email'))
                if user:
                    return render(request, "register.html", {"msg"+"_"+request.POST.get('user_type'):"Email is already registered."})

                job_seeker_form = JobSeekerForm(request.POST)
                if job_seeker_form.is_valid():
                    job_seeker = job_seeker_form.save()
                    print("User : ", job_seeker_form)
